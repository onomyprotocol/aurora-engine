...
impl Wei {
    const ETH_TO_WEI: U256 = U256::new([1_000_000_000_000_000_000, 0, 0, 0]);

    pub const fn zero() -> Self {
        Self(U256::new([0, 0, 0, 0]))
    }
    ...
    pub fn into_bytes(self) -> WeiBytes {
        self.0.to_be_bytes()
    }
...
impl From<WeiBytes> for Wei {
    fn from(value: WeiBytes) -> Self {
        Wei(U256::from_bytes(value))
    }
}
...
mod tests {
    ...
    fn test_wei_from_u64() {
        ...
        assert_eq!(Wei::new_u64(x).into_raw().as_u64(), x);
    }

    #[test]
    fn test_wei_from_eth() {
        let eth_amount: u64 = rand::random();
        let wei_amount = U256::new_u64(eth_amount).checked_mul(U256::new_u64(10).pow(U256::new_u64(18))).unwrap();
        assert_eq!(Wei::from_eth(U256::new_u64(eth_amount)), Some(Wei::new(wei_amount)));
    }
}
